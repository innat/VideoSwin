...
@keras.utils.register_keras_serializable(package="swin.transformer2.tiny.3d")
class SubClassVideoSwinT(keras.Model):
    def __init__(
            self,
        ...
        if pooling == "avg":
            self.pooling_layer = keras.layers.GlobalAveragePooling3D(name="avg_pool")
        elif pooling == "max":
            self.pooling_layer = keras.layers.GlobalMaxPooling3D(name="max_pool")
        else:
            raise ValueError(
    ...
    def build(self, input_shape):
        ...
        self.outputs.build(self.backbone.compute_output_shape(input_shape))
